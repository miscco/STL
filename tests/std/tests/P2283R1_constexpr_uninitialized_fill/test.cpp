...
struct instantiator {
    static constexpr int expected_output[] = {42, 42, 42};

    template <ranges::forward_range W>
    static constexpr void call() {
        using ranges::uninitialized_fill, ranges::common_range, ranges::equal;

        { // Validate range overload
            holder<int_wrapper, 3> mem;
            W wrapped_output{mem.as_span()};

            const auto result = uninitialized_fill(wrapped_output, 42);
        ...
        { // Validate iterator overload
            ...
            const auto result = uninitialized_fill(wrapped_output.begin(), wrapped_output.end(), 42);
            assert(result == wrapped_output.end());
            assert(equal(wrapped_output, expected_output));
        }
        ...
        if constexpr (common_range<W>) {
            ...
            const auto result = std::uninitialized_fill(wrapped_output.begin(), wrapped_output.end(), 42);
            assert(result == wrapped_output.end());
            assert(equal(wrapped_output, expected_output));
        }
    }
...
struct memset_test {
    static constexpr auto answer              = static_cast<unsigned char>(42);
    static constexpr unsigned char expected[] = {answer, answer, answer};

    static constexpr void call() {
        { // Validate ranges algorithm
            unsigned char input[3];

            const auto result = ranges::uninitialized_fill(input, answer);
            assert(result == end(input));
            assert(ranges::equal(input, expected));
        }

        { // Validate std algorithm
            unsigned char input[3];

            std::uninitialized_fill(begin(input), end(input), answer);
            assert(ranges::equal(input, expected));
        }
    }
};
...
int main() {
    ...
    static_assert((memset_test::call(), true));
}
