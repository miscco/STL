...
struct instantiator {
    static constexpr int expected_output[] = {10, 10, 10};

    template <ranges::forward_range W>
    static constexpr void call() {
        using ranges::uninitialized_value_construct, ranges::common_range, ranges::equal;

        { // Validate range overload
            holder<int_wrapper, 3> mem;
            W wrapped_output{mem.as_span()};

            const auto result = uninitialized_value_construct(wrapped_output);
        ...
        { // Validate iterator overload
            ...
            const auto result = uninitialized_value_construct(wrapped_output.begin(), wrapped_output.end());
            assert(result == wrapped_output.end());
            assert(equal(wrapped_output, expected_output));
        }
        ...
        if constexpr (common_range<W>) {
            ...
            const auto result = std::uninitialized_value_construct(wrapped_output.begin(), wrapped_output.end());
            assert(result == wrapped_output.end());
            assert(equal(wrapped_output, expected_output));
        }
    }
...
struct memset_test {
    ...
    static constexpr void call() {
        ...
        { // Validate std algorithm
            ...
            std::uninitialized_value_construct(begin(input), end(input));
            assert(ranges::equal(input, expected));
        }
    }
};
