...
struct instantiator {
    ...
    static constexpr void call() {
        using ranges::uninitialized_move, ranges::common_range, ranges::equal, ranges::range_value_t,
        ...
        { // Validate range overload
            ...
            const auto result = uninitialized_move(wrapped_input, wrapped_output);
        ...
        { // Validate iterator overload
            ...
            const auto result = uninitialized_move(
        ...
        if constexpr (common_range<R> && is_same_v<range_value_t<R>, remove_reference_t<range_reference_t<R>>>) {
            ...
            const auto result =
                std::uninitialized_move(wrapped_input.begin(), wrapped_input.end(), wrapped_output.begin());
        ...
        { // Validate range overload shorter output
            ...
            auto result = uninitialized_move(wrapped_input, wrapped_output);
            assert(++result.in == wrapped_input.end());
            assert(result.out == wrapped_output.end());
            assert(equal(wrapped_output, expected_output));
...
struct memcpy_test {
    ...
    static constexpr void call() {
        ...
        { // Validate matching range std algorithm
            ...
            const auto result = std::uninitialized_move(begin(input), end(input), begin(output));
        ...
        { // Validate input shorter std algorithm
            ...
            auto result = std::uninitialized_move(begin(input), end(input), begin(output));
            construct_at(addressof(*result), -1); // Need to construct non written element for comparison
            assert(++result == end(output));
            assert(ranges::equal(input, expected_input));
            assert(ranges::equal(output, expected_output_long));
