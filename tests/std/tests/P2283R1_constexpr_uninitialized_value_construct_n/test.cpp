...
struct instantiator {
    ...
    static constexpr void call() {
        using ranges::uninitialized_value_construct_n, ranges::common_range, ranges::equal;

        { // Validate iterator overload
            holder<int_wrapper, 3> mem;
            W wrapped_output{mem.as_span()};

            const auto result = uninitialized_value_construct_n(wrapped_output.begin(), 3);
        ...
        if constexpr (common_range<W>) {
            ...
            const auto result = std::uninitialized_value_construct_n(wrapped_output.begin(), 3);
            assert(result == wrapped_output.end());
            assert(equal(wrapped_output, expected_output));
        }
    }
...
struct memset_test {
    ...
    static constexpr void call() {
        ...
        { // Validate std algorithm
            ...
            const auto result = std::uninitialized_value_construct_n(begin(input), 3);
            assert(result == end(input));
            assert(ranges::equal(input, expected));
        }
    }
