...
struct instantiator {
    ...
    static constexpr void call() {
        using ranges::uninitialized_copy_n, ranges::common_range, ranges::equal, ranges::range_value_t,
        ...
        if constexpr (common_range<R> && is_same_v<range_value_t<R>, remove_reference_t<range_reference_t<R>>>) {
            ...
            const auto result = std::uninitialized_copy_n(wrapped_input.begin(), 3, wrapped_output.begin());
        ...
        { // Validate shorter output
            ...
            auto result = uninitialized_copy_n(wrapped_input.begin(), 3, wrapped_output.begin(), wrapped_output.end());
            assert(++result.in == wrapped_input.end());
            assert(result.out == wrapped_output.end());
            assert(equal(wrapped_output, expected_output));
...
struct memcpy_test {
    ...
    static constexpr void call() {
        { // Validate matching range
            ...
            const auto result = ranges::uninitialized_copy_n(begin(input), 3, begin(output), end(output));
        ...
        { // Validate matching range std algorithm
            ...
            const auto result = std::uninitialized_copy_n(begin(input), 3, begin(output));
        ...
        { // Validate output shorter
            ...
            auto result = ranges::uninitialized_copy_n(begin(input), 3, begin(output), end(output));
            assert(++result.in == end(input));
            assert(result.out == end(output));
        ...
        { // Validate input shorter std algorithm
            ...
            auto result = std::uninitialized_copy_n(begin(input), 3, begin(output));
            construct_at(addressof(*result), -1); // Need to construct non written element for comparison
            assert(++result == end(output));
            assert(ranges::equal(input, expected_input));
            assert(ranges::equal(output, expected_output_long));
